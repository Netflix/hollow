...
/**
 * Maps fields from Protocol Buffer messages to Hollow object fields.
 */
public class ObjectFieldMapping {

    private final HollowWriteStateEngine stateEngine;
    private final String typeName;
    private final HollowProtoAdapter adapter;
    ...
    public ObjectFieldMapping(String typeName, HollowProtoAdapter adapter) {
        this.stateEngine = adapter.stateEngine;
    ...
    private ObjectFieldMapping(String typeName, HollowProtoAdapter adapter, Map<String, ObjectMappedFieldPath> mappedFieldPaths, RemappingBuilderInstruction rootInstruction, Map<String, HollowObjectWriteRecord> writeRecords) {
        this.stateEngine = adapter.stateEngine;
        this.typeName = typeName;
        this.adapter = adapter;
    ...
    private void mapAllPaths(HollowObjectSchema schema) {
        for(int i=0;i<schema.numFields();i++) {
            if(!mappedFieldPaths.containsKey(schema.getFieldName(i))) {
                ...
                mappedFieldPaths.put(schema.getFieldName(i), new ObjectMappedFieldPath(rec, schema.getFieldName(i), schema.getName(), schema.getFieldName(i), i, adapter.getFieldProcessor(schema.getName(), schema.getFieldName(i))));
            }
        }
    }
    ...
    private FieldProcessor findFieldProcessor(String typeName, String fieldName, String mappedTypeName, String mappedFieldName) {
        FieldProcessor fp = adapter.getFieldProcessor(typeName, fieldName);
        if(fp != null)
            return fp;

        return adapter.getFieldProcessor(mappedTypeName, mappedFieldName);
    }
    ...
    public ObjectFieldMapping clone() {
        ...
        return new ObjectFieldMapping(typeName, adapter, clonedMappedFieldPaths, rootInstruction.clone(clonedWriteRecords), clonedWriteRecords);
    }
}
