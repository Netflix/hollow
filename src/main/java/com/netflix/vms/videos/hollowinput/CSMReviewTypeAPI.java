...
public class CSMReviewTypeAPI extends HollowObjectTypeAPI {
    ...
    public long getPlasticReleaseDate(int ordinal) {
        if(fieldIndex[5] == -1)
            return missingDataHandler().handleLong("CSMReview", ordinal, "plasticReleaseDate");
    ...
    public Long getPlasticReleaseDateBoxed(int ordinal) {
        long l;
        if(fieldIndex[5] == -1) {
            l = missingDataHandler().handleLong("CSMReview", ordinal, "plasticReleaseDate");
        } else {
    ...
    public long getReleaseDate(int ordinal) {
        if(fieldIndex[24] == -1)
            return missingDataHandler().handleLong("CSMReview", ordinal, "releaseDate");
        return getTypeDataAccess().readLong(ordinal, fieldIndex[24]);
    }

    public Long getReleaseDateBoxed(int ordinal) {
        long l;
        if(fieldIndex[24] == -1) {
            l = missingDataHandler().handleLong("CSMReview", ordinal, "releaseDate");
        } else {
            boxedFieldAccessSampler.recordFieldAccess(fieldIndex[24]);
            l = getTypeDataAccess().readLong(ordinal, fieldIndex[24]);
        }
        if(l == Long.MIN_VALUE)
            return null;
        return Long.valueOf(l);
