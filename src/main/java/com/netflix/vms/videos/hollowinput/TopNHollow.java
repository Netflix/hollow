...
public class TopNHollow extends HollowObject {
    ...
    public TopNArrayOfAttributesHollow _getAttributes() {
        int refOrdinal = delegate().getAttributesOrdinal(ordinal);
        if(refOrdinal == -1)
            return null;
        return  api().getTopNArrayOfAttributesHollow(refOrdinal);
    }

    public StringHollow _getDseSourceFile() {
        int refOrdinal = delegate().getDseSourceFileOrdinal(ordinal);
        if(refOrdinal == -1)
            return null;
