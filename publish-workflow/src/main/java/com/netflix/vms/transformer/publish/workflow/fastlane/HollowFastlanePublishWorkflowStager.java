...
import com.netflix.vms.transformer.publish.workflow.job.impl.S3HollowBlobPublishJob;
import java.io.File;
...
public class HollowFastlanePublishWorkflowStager implements PublishWorkflowStager {
    ...
    public HollowFastlanePublishWorkflowStager(TransformerContext ctx, FileStore fileStore, NetflixS3BlobPublisher blobPublisher, NetflixS3BlobPublisher nostreamsBlobPublisher, HermesBlobAnnouncer hermesBlobAnnouncer, Supplier<ServerUploadStatus> uploadStatus, String vip) {
    ...
    public CycleStatusFuture triggerPublish(long inputDataVersion, long previousVersion, long newVersion) {
        ...
        for(RegionEnum region : PublishRegionProvider.ALL_REGIONS) {
            createAnnounceJobForRegion(region, previousVersion, newVersion, publishJobs);
        }
    ...
    private List<PublicationJob> addPublishJobs(long inputDataVersion, long previousVersion, long newVersion) {
        ...
        if(snapshotFile.exists()){
            ...
            HollowBlobPublishJob publishJob = new S3HollowBlobPublishJob(ctx, inputDataVersion, previousVersion, newVersion, PublishType.SNAPSHOT, snapshotFile, false);
            scheduler.submitJob(publishJob);
            submittedJobs.add(publishJob);
        }
        if(deltaFile.exists()){
            HollowBlobPublishJob publishJob = new S3HollowBlobPublishJob(ctx, inputDataVersion, previousVersion, newVersion, PublishType.DELTA, deltaFile, false);
            scheduler.submitJob(publishJob);
            submittedJobs.add(publishJob);
        }
        if(reverseDeltaFile.exists()){
            HollowBlobPublishJob publishJob = new S3HollowBlobPublishJob(ctx, inputDataVersion, previousVersion, newVersion, PublishType.REVERSEDELTA, reverseDeltaFile, false);
            scheduler.submitJob(publishJob);
            submittedJobs.add(publishJob);
        }
