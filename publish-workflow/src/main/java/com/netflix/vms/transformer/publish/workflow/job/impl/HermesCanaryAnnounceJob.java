...
public class HermesCanaryAnnounceJob extends CanaryAnnounceJob {
    public HermesCanaryAnnounceJob(
            PublishWorkflowContext ctx, String vip, long newVersion,
    ...
    public boolean executeJob() {
        ...
        // It's okay to do global announcements, it would just mean all canary instances get the version to canary at
        // the same time)
        ctx.getCanaryAnnouncer().announce(getCycleVersion());

        boolean success = true;
        // announce canary version in all 3 regions as separate announcements (does not matter since we only run canary
        // in us-east-1)
