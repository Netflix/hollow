...
import com.netflix.hollow.core.read.engine.HollowReadFileEngine;
 ...
 */
public class HollowObjectTypeReadFile extends HollowTypeReadFile implements HollowObjectTypeDataAccess {
    ...
    private final HollowObjectTypeReadFileShard shards[];

    private int maxOrdinal;

    public HollowObjectTypeReadFile(HollowReadFileEngine fileEngine, HollowObjectSchema schema) {
        this(fileEngine, schema, schema, 1);
    }

    public HollowObjectTypeReadFile(HollowReadFileEngine fileEngine, HollowObjectSchema schema, HollowObjectSchema unfilteredSchema, int numShards) {
        ...
        HollowObjectTypeReadFileShard shards[] = new HollowObjectTypeReadFileShard[numShards];
        for(int i=0;i<shards.length;i++)
            shards[i] = new HollowObjectTypeReadFileShard(schema);
    ...
    public void readSnapshot(RandomAccessFile raf, ArraySegmentRecycler memoryRecycler) throws IOException {
        ...
        for(int i=0;i<shards.length;i++) {
            HollowObjectTypeFileElements snapshotData = new HollowObjectTypeFileElements(getSchema(), memoryRecycler);
