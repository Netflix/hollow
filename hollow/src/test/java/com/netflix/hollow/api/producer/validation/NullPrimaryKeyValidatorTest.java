...
public class NullPrimaryKeyValidatorTest {
    ...
    public void testValidPrimaryKey() {
        HollowProducer producer = HollowProducer.withPublisher(blobStore)
                ...
                .withListener(new NullPrimaryKeyValidator(TypeWithSinglePrimaryKey.class)).build();
        try {
    ...
    public void testInvalidNoSchema() {
        ...
        try {
            ...
            assertEquals("NullPrimaryKeyValidator defined for data type TypeWithSinglePrimaryKey "+
    ...
    public void testInvalidNoPrimaryKey() {
        HollowProducer producer = HollowProducer.withPublisher(blobStore)
                .withBlobStager(new HollowInMemoryBlobStager())
                .withListener(new NullPrimaryKeyValidator("TypeWithoutPrimaryKey")).build();
        try {
            producer.runCycle(state -> {
            ...
            ValidationStatusException expected = (ValidationStatusException) e;
            assertEquals(1, expected.getValidationStatus().getResults().size());
            assertEquals("NullPrimaryKeyValidator defined but unable to find primary key "+
    ...
    public void testInvalidNullMultiPrimaryKeys() {
        HollowProducer producer = HollowProducer.withPublisher(blobStore)
                .withBlobStager(new HollowInMemoryBlobStager())
                .withListener(new NullPrimaryKeyValidator(TypeWithMultiplePrimaryKeys.class)).build();
        try {
            producer.runCycle(state -> {
    ...
    public void testInvalidNullNestedPrimaryKey() {
        HollowProducer producer = HollowProducer.withPublisher(blobStore)
                .withBlobStager(new HollowInMemoryBlobStager())
                .withListener(new NullPrimaryKeyValidator(TypeWithNestedPrimaryKey.class)).build();
