...
public class HollowCollectionTypeNameTest extends AbstractStateEngineTest {
    ...
    static class TypeWithCollectionAnnotationOnInt {
        @HollowCollectionTypeName(elementTypeName = "SomeName")
        int id;
    }

    static class TypeWithMapAnnotationOnLong {
        @HollowMapTypeName(keyTypeName = "K")
        long id;
    }

    static class TypeWithCollectionAnnotationOnInlinedInteger {
        @HollowInline
        @HollowCollectionTypeName(elementTypeName = "SomeName")
        Integer id;
    }
    ...
    // -------------------------------------------------------------------------
    // Both annotations on the same field (issue 4 — clear error message)
    // -------------------------------------------------------------------------
    ...
    public void flagOn_bothAnnotationsOnListField_errorMentionsBothAnnotations() {
        ...
        try {
            ...
            Assert.assertTrue("Error should mention @HollowCollectionTypeName",
                    e.getMessage().contains("@HollowCollectionTypeName"));
            Assert.assertTrue("Error should mention @HollowMapTypeName",
                    e.getMessage().contains("@HollowMapTypeName"));
        }
    }

    static class TypeWithBothAnnotationsOnList {
        @HollowCollectionTypeName(elementTypeName = "Elem")
        @HollowMapTypeName(keyTypeName = "K")
        List<Integer> ids;
    }

    static class TypeWithBothAnnotationsOnMap {
        @HollowCollectionTypeName(elementTypeName = "Elem")
        @HollowMapTypeName(keyTypeName = "K")
        Map<String, String> data;
    }
    ...
    // -------------------------------------------------------------------------
    // Empty string elementTypeName / keyTypeName falls back to auto-generated name (issue 7)
    // -------------------------------------------------------------------------
    ...
    public void flagOn_emptyElementTypeName_usesAutoGeneratedName() {
        ...
        HollowListSchema schema = (HollowListSchema) writeStateEngine.getTypeState("ListOfInteger").getSchema();
        Assert.assertEquals("Integer", schema.getElementType());
        Assert.assertNull("No custom type state should exist", writeStateEngine.getTypeState(""));
    }
    ...
    public void flagOn_emptyValueTypeName_onlyKeyIsNamed() {
        ...
        HollowMapSchema schema = (HollowMapSchema) writeStateEngine.getTypeState("MapOfStringToString").getSchema();
        Assert.assertEquals("NamedKey", schema.getKeyType());
        Assert.assertEquals("String", schema.getValueType());
    }

    static class TypeWithEmptyElementTypeName {
        @HollowCollectionTypeName(elementTypeName = "")
        List<Integer> ids;
    }

    static class TypeWithEmptyKeyTypeName {
        @HollowMapTypeName(keyTypeName = "", valueTypeName = "")
        Map<String, String> data;
    }

    static class TypeWithEmptyValueTypeName {
        @HollowMapTypeName(keyTypeName = "NamedKey", valueTypeName = "")
        Map<String, String> data;
    }
}
