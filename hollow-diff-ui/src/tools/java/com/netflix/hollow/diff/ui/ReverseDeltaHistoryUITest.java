...
public class ReverseDeltaHistoryUITest {
    ...
    public void matchDeltaWithReverseDelta() throws Exception {
        ...
        for(int j=0; j<historyR.getHistoricalStates().length; j++) {
            ...
            // SNAP: assertEquals("Version should match", stateD.getVersion(), stateR.getVersion());
            ...
            // SNAP: assertEquals("Not same next version", getNextStateVersion(stateD), getPreviousStateVersion(stateR, historyR));
            // SNAP: assertEquals("Not same next version", getNextStateVersion(stateR), getPreviousStateVersion(stateD, historyD));
