...
public class MovieExtensionOverrideDelegateLookupImpl extends HollowObjectAbstractDelegate implements MovieExtensionOverrideDelegate {

    private final MovieExtensionOverrideTypeAPI typeAPI;

    public MovieExtensionOverrideDelegateLookupImpl(MovieExtensionOverrideTypeAPI typeAPI) {
        this.typeAPI = typeAPI;
    }

    public String getEntityType(int ordinal) {
        ordinal = typeAPI.getEntityTypeOrdinal(ordinal);
        return ordinal == -1 ? null : typeAPI.getAPI().getOverrideEntityTypeTypeAPI().get_name(ordinal);
    }

    public boolean isEntityTypeEqual(int ordinal, String testValue) {
        ordinal = typeAPI.getEntityTypeOrdinal(ordinal);
        return ordinal == -1 ? testValue == null : typeAPI.getAPI().getOverrideEntityTypeTypeAPI().is_nameEqual(ordinal, testValue);
    }

    public int getEntityTypeOrdinal(int ordinal) {
        return typeAPI.getEntityTypeOrdinal(ordinal);
    }

    public String getEntityValue(int ordinal) {
        ordinal = typeAPI.getEntityValueOrdinal(ordinal);
        return ordinal == -1 ? null : typeAPI.getAPI().getOverrideEntityValueTypeAPI().getValue(ordinal);
    }

    public boolean isEntityValueEqual(int ordinal, String testValue) {
        ordinal = typeAPI.getEntityValueOrdinal(ordinal);
        return ordinal == -1 ? testValue == null : typeAPI.getAPI().getOverrideEntityValueTypeAPI().isValueEqual(ordinal, testValue);
    }

    public int getEntityValueOrdinal(int ordinal) {
        return typeAPI.getEntityValueOrdinal(ordinal);
    }
