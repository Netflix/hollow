...
public class PhaseRequiredImageTypeDelegateLookupImpl extends HollowObjectAbstractDelegate implements PhaseRequiredImageTypeDelegate {

    private final PhaseRequiredImageTypeTypeAPI typeAPI;

    public PhaseRequiredImageTypeDelegateLookupImpl(PhaseRequiredImageTypeTypeAPI typeAPI) {
        this.typeAPI = typeAPI;
    }

    public String getImageType(int ordinal) {
        ordinal = typeAPI.getImageTypeOrdinal(ordinal);
        return ordinal == -1 ? null : typeAPI.getAPI().getImageTypeTypeAPI().getValue(ordinal);
    }

    public boolean isImageTypeEqual(int ordinal, String testValue) {
        ordinal = typeAPI.getImageTypeOrdinal(ordinal);
        return ordinal == -1 ? testValue == null : typeAPI.getAPI().getImageTypeTypeAPI().isValueEqual(ordinal, testValue);
    }
    ...
    public boolean getImageSwapRequired(int ordinal) {
        return typeAPI.getImageSwapRequired(ordinal);
    }

    public Boolean getImageSwapRequiredBoxed(int ordinal) {
        return typeAPI.getImageSwapRequiredBoxed(ordinal);
    }
    ...
    public PhaseRequiredImageTypeTypeAPI getTypeAPI() {
        return typeAPI;
    }
