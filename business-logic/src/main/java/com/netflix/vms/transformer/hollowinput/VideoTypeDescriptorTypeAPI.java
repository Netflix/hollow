...
public class VideoTypeDescriptorTypeAPI extends HollowObjectTypeAPI {
    ...
    public boolean getOriginal(int ordinal) {
        if(fieldIndex[3] == -1)
            return missingDataHandler().handleBoolean("VideoTypeDescriptor", ordinal, "original") == Boolean.TRUE;
    ...
    public boolean getExtended(int ordinal) {
        if(fieldIndex[5] == -1)
            return missingDataHandler().handleBoolean("VideoTypeDescriptor", ordinal, "extended") == Boolean.TRUE;
